<template>
	<view class="container">
		<navbar title="待提交"></navbar>
		<view class="content">
			<view class="search">
				<view class="left" @click="state.pub_picker=true">
					<text style="margin-right: 1.33vw;">{{state.search_type.name}}</text>
			...
			<view class="list" v-for="(item,index) in state.list" :key="index"
				@click="$navigateTo('/home_packages/details/index',{order_id:item.id,id:item.goods_id,type:state.type[item.type]})">
				<view class="title">
					<view class="left">
