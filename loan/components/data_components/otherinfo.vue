				...
				<view class="value" style="border-bottom: none;">
					<view class="idcard" v-if="!props.showFlag" v-for="(item,index) in state.id_card_array"
						...
						<view class="item" @click="uploadIdImg('front')">
							点击上传身份证正面
							<text>（人像）</text>
							<image :src="item.front" mode="" v-if="item.front"></image>
						</view>
						<view class="item" @click="uploadIdImg('back')">
							点击上传身份证背面
							<text>（国徽）</text>
							<image :src="item.back" mode="" v-if="item.back"></image>
						</view>
					</view>
					<image style="width: 40.8vw;height: 22.93vw;" :src="item" @click="preImg(item)" mode=""
						v-for="(item,index) in formData.materials.id_card_image" :key="index" v-else></image>
			...
			<view class="info_item">
				...
				<view class="value" style="border-bottom: none;">
					<view class="imgs" v-for="(item,index) in formData.materials.hukou_image" :key="index">
						<image :src="item" mode="" @click="preImg(item)"></image>
...
<script setup lang="ts">
	...
	if (uni.getStorageSync("setInfoFormData")) {
		state.id_card_array = [{
			front: uni.getStorageSync("setInfoFormData").card_image_front,
			back: uni.getStorageSync("setInfoFormData").card_image_back
		}]
	}
