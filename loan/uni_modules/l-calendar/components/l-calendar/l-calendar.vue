<template>
	<view @touchmove.stop.prevent>
		<view class="l-calendar-box" :class="{'calendar-box-show': visible}">
			...
			<view class="date-box">
				...
				<view class="card" @tap="dateClick(-1)" v-if="idCard">长期</view>
			</view>
			...
			<view class="calendar-text">
				...
				<view class="calendar-btn">
					<view :style="{opacity:disabled ? '.5' : '1'}" :disabled="disabled" @click="confireBtnClick(false)">
						确定
					</view>
				</view>
			</view>
		...
		<view class="mask" :class="[visible ? 'mask-show' : '']" @tap="hide"></view>
	</view>
</template>
<script>
	...
	export default {
		...
		props: {
			...
			maxDate: {
				type: String,
				default: new Date().getFullYear() + 100 + "-12-31"
			},
			//组件标题
			title: {
				type: String,
