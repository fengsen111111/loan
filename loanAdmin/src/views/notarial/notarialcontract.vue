	...
	<div class="gi_page">
		<a-card>
			<GiTable row-key="id" :data="dataList" :columns="columns" :loading="loading" :row-selection="rowSelection"
				@selection-change="selectionChange" :scroll="{ x: '100%', y: '100%', minWidth: 3000 }"
				:pagination="pagination" :disabledTools="['fullscreen', 'refresh', 'size', 'setting']"
				@refresh="search">
					...
					<a-row class="grid-demo2" :gutter="24">
						...
						<a-col :span="8">
							<div style="display: flex">
								...
								<a-input v-model="queryForm.user_name" placeholder="请输入联系人" allow-clear
									@change="search">
									<template #prefix></template>
								</a-input>
							</div>
						...
						<a-col :span="8" v-if="isExpanded">
							<div style="display: flex">
								...
								<a-cascader :options="GoodstypeList" v-model="queryForm.goods_type"
									placeholder="请选择产品类型" @change="search" />
							</div>
						</a-col>
						...
						<a-col :span="8" v-if="isExpanded">
							<div style="display: flex">
								...
								<a-cascader :options="checkStatusList" v-model="queryForm.check_status"
									placeholder="请选择子进度" @change="search" />
							</div>
						</a-col>
						<a-col :span="8" v-if="isExpanded">
							<div style="display: flex">
						...
						<a-col :span="8" v-if="isExpanded">
							<div style="display: flex">
								...
								<a-input v-model="queryForm.user_mobile" placeholder="请输入联系电话" allow-clear
									@change="search">
									<template #prefix></template>
								</a-input>
							</div>
            ...
            record.status == 'a'
              ...
              : record.status == 'b'
				...
				<!-- 签约链接 -->
				<template #notarization_urls="{ record }">
					<div>
						<div v-for="(url, name) in record.notarization_urls" :key="name" style="word-break: break-all; white-space: normal;">
							{{ name }}: {{ url }}
						</div>
					</div>
				</template>
				...
				<template #action="{ record }">
					<div>
						<a-space>
							...
							<a-popconfirm content="确定要执行该操作 ?" ok-text="确定" cancel-text="取消" @ok="zuofei(record)">
								<a-link> 作废 </a-link>
							</a-popconfirm>
						</a-space>
						<a-popconfirm content="确定要执行该操作 ?" ok-text="确定" cancel-text="取消" @ok="_getHtmlUrl(record)">
							<a-link> 获取签约链接 </a-link>
						</a-popconfirm>
					</div>
				</template>
			</GiTable>
			...
			<div>
				<a-row class="grid-demo1">
					...
					<a-col :span="24">
						<div>
							<span>驳回理由 </span>
							<a-textarea placeholder="请输入驳回理由" v-model="reject_des" :max-length="255" allow-clear
								show-word-limit />
						</div>
					</a-col>
				</a-row>
...
<script setup lang="ts">
	...
	const columns : TableInstanceColumns[] = [
		...
		{ title: '签约链接', align: 'left', slotName: 'notarization_urls', ellipsis: true, tooltip: true,width: 300, },
		{
			title: '操作',
			slotName: 'action',
			align: 'center',
