		...
		<view class="content">
			...
			<view class="list" v-for="(item,index) in state.list" :key="index">
				<view class="title">
					<view class="left">
						<view class="info"
							:style="item.type==='person'?'max-width:15vw;min-width:14vw;width:fit-content':'max-width:40vw;min-width:17vw;width:fit-content'">
							{{item.name}}
						</view>
						{{item.type==='person'?$cardUtil(item.card_number):''}}
						<view v-if="item.delete_status=='Y'">
							<view class="type" style="color: #999999;border:1px solid #999999;border-radius: 8rpx;padding: 0 20rpx;font-size: 20rpx;margin-left: 10rpx;font-weight: bold;"
								v-if="item.status == 'b'||item.status == 'c'||item.status == 'e'">
								已作废
							</view>
							<view v-else class="type" style="border-radius: 8rpx;padding: 0 20rpx;font-size: 20rpx;margin-left: 10rpx;font-weight: bold;"
				...
				<view class="con">
					...
					<!-- &&state.appType===1 -->
					<view class="rightText" v-if="item.delete_status!=='Y'">
						<view @click="sqmimy(item.id)"
							v-if="item.open_ll_status==='Y'&&item.ll_account_status == 'b' && item.jump_pass_status == 'c'">
							申请提额
						</view>
					</view>
				</view>
				<view class="con">
					...
					<view class="rightText" v-if="item.delete_status!=='Y'">
						...
						<view
							...
							style="display: flex;">
							<view @click="copyHref(item.manage_ll_account_url)" style="margin-left: 0rpx;">
								账户管理
							</view>
						</view>
						<!-- <view class="btn">更换银行卡</view> -->
					</view>
				</view>
			</view>
...
<script setup lang="ts">
	...
	const netWork = (type : number) => {
		...
		globalProxy.$request("/loan/Order/getOrderList", {
			...
			user_type: uni.getStorageSync("appType") == '2' ? 'b' : 'a',
