...
const useCopyCode = (props, html, key) => {
  ...
  const initCopyEntry = () => {
    document.querySelectorAll(`#${editorId} .${config.prefix}-preview .${config.prefix}-code`).forEach((codeBlock) => {
      let clearTimer = -1;
...
const useMermaid = (props) => {
  ...
  vue.onMounted(() => {
    var _a, _b, _c;
    if (props.noMermaid || ((_a = mermaidRef.value) == null ? void 0 : _a.instance)) {
      return;
    }
...
const codetabs = (md, _opts) => {
  const defaultRender = md.renderer.rules.fence, unescapeAll = md.utils.unescapeAll, re = /\[(\w*)(?::([\w ]*))?\]/, mandatoryRe = /::close/;
  const getInfo = (token) => {
  ...
  const getTagType = (token) => {
    ...
    const open = !mandatory && token.content.trim().split("\n").length < _opts.autoFoldThreshold;
    const tagContainer = mandatory || _opts.codeFoldable ? "details" : "div", tagHeader = mandatory || _opts.codeFoldable ? "summary" : "div";
  ...
  const fenceGroup = (tokens, idx, options, env, slf) => {
    ...
    <${tagHeader2} class="${config.prefix}-code-head">
      ...
      <div>
        ...
        <span class="${config.prefix}-copy-button">${(_a = _opts.usedLanguageTextRef.value) == null ? void 0 : _a.copyCode.text}</span>
      </div>
    ...
    for (let i = idx; i < tokens.length; i++) {
      ...
      labels += `<li>
          <input type="radio" name="${config.prefix}-codetab-label-${_opts.editorId}-${idx}" class="${className}" ${checked}>
          <label onclick="document.querySelectorAll('.${className}').forEach(e => e.click())">${tab || getLangName(token)}</label>
        </li>`;
      pres += `<input type="radio" name="${config.prefix}-codetab-pre-${_opts.editorId}-${idx}" class="${className}" ${checked}>
      ${defaultRender(tokens, i, options, env, slf)}`;
      langs += `<input type="radio" name="${config.prefix}-codetab-lang-${_opts.editorId}-${idx}" class="${className}" ${checked}>
      <span class=${config.prefix}-code-lang>${getLangName(token)}</span>`;
    }
    return `<${tagContainer} ${slf.renderAttrs(tmpToken)}>
    <${tagHeader} class="${config.prefix}-code-head">
      <div class="${config.prefix}-code-flag">
        <ul class="${config.prefix}-codetab-label">${labels}</ul>
      </div>
      <div>
        <span class="${config.prefix}-codetab-lang">${langs}</span>
        <span class="${config.prefix}-copy-button">${(_b = _opts.usedLanguageTextRef.value) == null ? void 0 : _b.copyCode.text}</span>
      </div>
    </${tagHeader}>${pres}</${tagContainer}>`;
  };
  md.renderer.rules.fence = fenceGroup;
};
const CodePlugin = codetabs;
...
const useMarkdownIt = (props, previewOnly) => {
  ...
  vue.onMounted(() => {
    ...
    eventName.bus.on(editorId, {
      ...
      callback: () => {
        key.value = `_article-key_${util$2.uuid()}`;
        markHtml();
      }
    });
  });
