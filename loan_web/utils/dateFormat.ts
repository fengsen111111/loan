export function dateFormat(date:any,formatter:any,isPad = true){
  date = new Date(date);
...
function formatNormalize(formatter:any){
  ...
  return (dateInfo: any) => {
    ...
    return formatter.replaceAll("yyyy", yyyy).replaceAll("MM", MM).replaceAll("dd", dd).replaceAll("HH", HH).replaceAll("mm", mm).replaceAll("ss", ss);
  };
}
